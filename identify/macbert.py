...
class MyModel(torch.nn.Module):
    def __init__(self, num_labels, dropout_prob):
        ...
        self.macbert = AutoModel.from_pretrained(model_path)
        self.dropout = torch.nn.Dropout(dropout_prob)
        self.classifier = torch.nn.Linear(self.macbert.config.hidden_size, num_labels)

    def forward(self, input_ids, attention_mask, labels=None):
        outputs = self.macbert(
            input_ids=input_ids,
...
if __name__ == '__main__':
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # 写入日志
        with open(f'../logs/identify/3_all_macbert_{num_epochs}.txt', 'a') as f:
        ...
        if epoch % draw_step == 0:
            plot_loss_acc(train_losses, test_losses, train_accuracies, test_accuracies, epoch,
                path=f'../training_curves/identify/3_all_macbert_{epoch}.png'
            )
